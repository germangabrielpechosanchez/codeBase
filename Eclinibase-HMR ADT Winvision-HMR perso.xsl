...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
      ...
      <xsl:element name="PV1">
         ...
         <xsl:element name="PV1.10.2">
            ...
            <xsl:choose>
               ...
               <xsl:when test="$hospitalService ='MURG'">  
                  <xsl:value-of select="$medecineUrgence"/> 
               </xsl:when> 
         ...
         <xsl:element name="PV1.14.2"> 
            ...
            <xsl:choose>
               ...
               <xsl:when test="$admissionType ='6'">  
                  <xsl:value-of select="$inscSounDUnJour"/> 
               </xsl:when>  
      ...
      <xsl:variable name="patientType" select="/HL7/PV1/PV1.18.1" />
        ...
        <xsl:if test="$dischargeDisposition != ''">     
         <xsl:element name="PV1.36.1">  
           ...
           <xsl:element name="PV1.37.1"> 
              <xsl:value-of select="/HL7/PV1/PV1.37.1"/> 
           </xsl:element> 
         </xsl:if>  
   ...
   <xsl:template name="ZPV">
     ...
     <xsl:variable name="programmeEchange">PROGRAMME D'ECHANGE</xsl:variable>
      ...
      <xsl:element name="ZPV.2.2">
         <xsl:choose>
            ...
            <xsl:when test="($citizenship = '6')"> 
               <xsl:value-of select="$programmeEchange"/>
            </xsl:when> 
            
            <xsl:when test="($citizenship = '7')"> 
               <xsl:value-of select="'PERMIS MINISTÉRIEL'"/>
            </xsl:when> 
            
            <xsl:when test="($citizenship = '8')"> 
               <xsl:value-of select="'REFUGIÉ POLITIQUE'"/>
            </xsl:when> 
            
            <xsl:when test="($citizenship = '9')"> 
               <xsl:value-of select="'DIPLOMATE ET EMPLOYÉS(ES) DE CONSULAT'"/>
            </xsl:when> 
